...
def test_xml_mpu_container_constructor_and_properties(filename):
    ...
    assert container.upload_id == None
    assert container._headers == {}
    assert container._parts == {}
    assert container._filename == filename

    container = _upload.XMLMPUContainer(EXAMPLE_XML_UPLOAD_URL, filename, headers=EXAMPLE_HEADERS, upload_id=UPLOAD_ID, parts=PARTS)
    assert container.upload_url == EXAMPLE_XML_UPLOAD_URL
...
def test_xml_mpu_container_initiate(filename):
    container = _upload.XMLMPUContainer(EXAMPLE_XML_UPLOAD_URL, filename, upload_id=UPLOAD_ID)
    with pytest.raises(ValueError):
        container._prepare_initiate_request(BASIC_CONTENT)

    container = _upload.XMLMPUContainer(EXAMPLE_XML_UPLOAD_URL, filename, headers=EXAMPLE_HEADERS)
    ...
    response = _make_xml_response(text=EXAMPLE_XML_MPU_INITIATE_TEXT_TEMPLATE.format(upload_id=UPLOAD_ID))
...
def test_xml_mpu_part(filename):
    ...
    START = 0
    END = 256
    part = _upload.XMLMPUPart(EXAMPLE_XML_UPLOAD_URL, UPLOAD_ID, filename, START, END, PART_NUMBER, headers=EXAMPLE_HEADERS, checksum='md5')
    ...
    assert part.end == END
    assert part._part_number == PART_NUMBER
    assert part._headers == EXAMPLE_HEADERS
    assert part._checksum_type == 'md5'
    assert part._checksum_object == None

    part = _upload.XMLMPUPart(EXAMPLE_XML_UPLOAD_URL, UPLOAD_ID, filename, START, END, PART_NUMBER, headers=EXAMPLE_HEADERS)
    verb, url, payload, headers = part._prepare_upload_request()
    assert verb == _upload._PUT
    assert url == EXAMPLE_XML_UPLOAD_URL + _upload._MPU_PART_QUERY_TEMPLATE.format(part=PART_NUMBER, upload_id=UPLOAD_ID)
    assert headers == EXAMPLE_HEADERS
    ...
    response = _make_xml_response(headers={'etag': ETAG, 'x-goog-hash': 'md5=Ojk9c3dhfxgoKVVHYwFbHQ=='}) # Example md5 checksum but not the correct one
    ...
    _fix_up_virtual(part)
    part._prepare_upload_request()
    response = _make_xml_response(headers={'etag': ETAG, 'x-goog-hash': 'md5=pOUFGnohRRFFd24NztFuFw=='}) # Example md5 checksum but not the correct one
    part._process_upload_response(response)
    assert part.etag == ETAG
